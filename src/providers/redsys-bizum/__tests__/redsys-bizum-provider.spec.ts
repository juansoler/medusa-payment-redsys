...
describe("RedsysBizumProviderService", () => {
  ...
  describe("initiatePayment", () => {
    it("creates a redirect form with DS_MERCHANT_PAYMETHODS z and webhookConfirmed false", async () => {
  ...
  describe("authorizePayment", () => {
    it("returns PENDING without webhook confirmation", async () => {
      const { service } = createService()
    ...
    it("returns PENDING even if status is authorized but webhook not confirmed", async () => {
      const { service } = createService()
    ...
    it("returns CAPTURED for a confirmed immediate capture", async () => {
      const { service } = createService()
    ...
    it("returns AUTHORIZED for a confirmed preauthorization", async () => {
  ...
  describe("updatePayment", () => {
    it("generates a new order id, preserves the session id and resets webhookConfirmed", async () => {
      ...
      expect(result.data.webhookConfirmed).toBe(false)
      expect(result.data.orderId).not.toBe(ORDER_ID)

      const params = mockRedsysApi.createRedirectForm.mock.calls[0][0]
      expect(params.DS_MERCHANT_PAYMETHODS).toBe("z")
      expect(params.DS_MERCHANT_MERCHANTDATA).toBe(
        `cart_1|${SESSION_ID}|${result.data.orderId}`
      )
    })
  })
